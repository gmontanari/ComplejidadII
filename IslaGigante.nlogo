...
to find-all-components
  ...
  loop
  [
    let start one-of turtles with [ explored? = false]
    if start = nobody [stop]
    set component-size 0
    ask start [explore blue - 10]
...
to go
  ...
  [
    user-message "Fully connected network"
    stop
  ]
  add-edge
  find-all-components
...
to add-edge
  let n1 one-of turtles
  let n2 one-of turtles
  ask n1
  [
   ifelse n1 = n2 or link-neighbor? n2
    [add-edge]
    [create-link-with n2]
  ]
end
...
"" ""
PENS
"K" 1.0 0 -8630108 true "" "plotxy (mean [count link-neighbors] of turtles) giant-component-size / N"
